document.addEventListener('DOMContentLoaded', function () {
  ...
  const airportListContainer = document.getElementById('airportList');
  ...
  searchInput.addEventListener('input', function () {
    renderAirportList(airportData, searchInput.value.toLowerCase());
  });

  // Renderizar la lista de aeropuertos al cargar la página
  renderAirportList(airportData);
});

// Función para renderizar la lista de aeropuertos
function renderAirportList(data, searchQuery = '') {
  // Obtener referencia al contenedor de la lista de aeropuertos
  const airportListContainer = document.getElementById('airportList');
  ...
  filteredData.forEach((airport) => {
    ...
    airportItem.addEventListener('click', () => {
      ...
      if (regexResult && regexResult[1]) {
        ...
        insertData("Desde: (" + valorDentroParentesis + ") (" + valorFueraParentesis + ")");
      }
    });
  });
}
